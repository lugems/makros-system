...
export function PlantTable({ plants, customers, onUpdateMeter, onEdit }: PlantTableProps) {
  ...
  return (
    <div className="rounded-2xl sm:rounded-3xl border bg-card overflow-hidden shadow-sm premium-shadow">
      {/* Mobile Cards View */}
        ...
        {plants.map((plant) => {
          ...
          return (
            <div 
              ...
              className="p-4 space-y-3.5 hover:bg-muted/20 transition-colors cursor-pointer active:bg-muted/30"
            >
              {/* Header: Identity, Model, Status & Actions */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="h-10 w-10 rounded-2xl bg-primary/5 border border-primary/10 flex items-center justify-center text-primary shrink-0">
                    <Hammer className="h-5 w-5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black uppercase tracking-tight text-foreground truncate">{plant.name}</p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest truncate">{plant.make} {plant.model}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                  <Badge variant="outline" className={cn(
                    "text-[8px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-lg",
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-2xl p-2 w-60 shadow-2xl border-border/50">
                      ...
                      <DropdownMenuItem onClick={() => router.push(`/job-cards/new?assetId=${plant.id}&assetType=Plant`)} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <Plus className="h-4 w-4 text-primary" /> Create Work Order
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => onUpdateMeter(plant)} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <Gauge className="h-4 w-4 text-indigo-500" /> Update Telemetry
                      </DropdownMenuItem>
                      
                      <DropdownMenuSeparator className="opacity-50" />
                      
                      <DropdownMenuItem onClick={() => router.push(`/plants-equipment/${plant.id}`)} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <Eye className="h-4 w-4 text-primary" /> Technical Dossier
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => onEdit(plant)} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <Edit className="h-4 w-4 text-primary" /> Edit Record
                      </DropdownMenuItem>
                      
                      <DropdownMenuSeparator className="opacity-50" />
                      
                      <DropdownMenuItem onClick={() => handleStatusShift(plant.id, 'Under Repair')} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <Activity className="h-4 w-4 text-orange-500" /> Mark Under Repair
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleStatusShift(plant.id, 'Out of Service')} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest">
                        <PowerOff className="h-4 w-4 text-red-500" /> Out of Service
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => setPlantToDecommission(plant)} className="rounded-lg gap-3 py-2 text-[10px] font-black uppercase tracking-widest text-destructive">
              ...
              {/* Asset Reference & Telemetry */}
              <div className="grid grid-cols-2 gap-2 bg-muted/30 p-2.5 rounded-xl border border-border/40 text-xs">
                <div className="space-y-0.5">
                  <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground block">Asset Reference</span>
                  <div className="flex items-center gap-1">
                    <Fingerprint className="h-3 w-3 text-primary opacity-40 shrink-0" />
                    <span className="font-mono font-black text-[10px] text-foreground/90 truncate">{plant.assetId}</span>
                  </div>
                  <p className="text-[8px] text-muted-foreground uppercase">S/N: {plant.serialNumber}</p>
                </div>
                <div className="space-y-0.5">
                  <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground block">Telemetry</span>
                  <div className="flex items-center gap-1.5">
                    <Gauge className="h-3 w-3 text-indigo-500 shrink-0" />
                    <span className="font-black text-[11px] tabular-nums">{plant.meterReading.toLocaleString()}</span>
                    <span className="text-[8px] font-black text-muted-foreground uppercase">{unit}</span>
                  </div>
                </div>
              </div>

              {/* Owner Authority Footer */}

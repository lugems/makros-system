        ...
        <div className="rounded-2xl border bg-card overflow-hidden shadow-sm">
            ...
            <div className="block md:hidden divide-y divide-border/50">
                {staff.map((s) => (
                    ...
                    >
                        {/* Top row: Avatar, Name & ID, Role Badge */}
                        <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="space-y-0.5 min-w-0 flex-1">
                                    <p className="text-sm font-black uppercase tracking-tight text-foreground truncate leading-tight group-hover:text-primary transition-colors">
                                        {s.fullName}
                                    </p>
                        ...
                        {/* Bottom Row: Status badge, Workload (if mechanic), and Action Menu */}
                        <div className="flex items-center justify-between pt-2 border-t border-border/40 gap-2">
                            <div className="flex items-center gap-2">
                                <Badge variant={s.status === 'Active' ? 'success' : 'destructive'} className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 shadow-sm">
                                    {s.status}
                                </Badge>
                                {s.role === 'Mechanic' && (
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                                        Bay: <strong className="text-foreground">{s.assignedJobs || 0}</strong> active
                                    </span>
                                )}
                            </div>
                ...
                {staff.length === 0 && (
                    <div className="py-16 text-center opacity-40 italic text-xs text-muted-foreground flex flex-col items-center">
                        ...
                        No personnel records match the active criteria.
                    </div>
                )}
            </div>

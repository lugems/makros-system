...
export function CommunicationsTable({ logs, onPreview, onEdit }: CommunicationsTableProps) {
  ...
  return (
    <div className="rounded-3xl border bg-card overflow-hidden shadow-sm premium-shadow">
      {/* Mobile Interaction Trace Cards View */}
      <div className="block md:hidden divide-y divide-border/50">
        {logs.map((log) => (
          <div
            key={log.logId}
            onClick={() => onPreview(log)}
            className="p-4 space-y-3 hover:bg-muted/20 transition-colors cursor-pointer active:bg-muted/30"
          >
            {/* Top row: Personnel & Direction/Status */}
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="space-y-0.5 min-w-0 flex-1">
                  <p className="text-xs font-black uppercase tracking-tight text-foreground truncate leading-tight">
                    {log.fromName || 'SYSTEM'}
                  </p>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <Badge
                  variant="outline"
                  className={cn(
                    "text-[7px] font-black uppercase tracking-widest h-5 px-1.5",
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <p className="text-xs font-black uppercase tracking-tight text-foreground truncate group-hover:text-primary transition-colors">
                  {log.subject}
                </p>
                {log.isInternalOnly && <Lock className="h-3 w-3 text-muted-foreground/40 shrink-0" />}
              </div>
              {log.message && (
            ...
            <div className="flex flex-wrap items-center gap-2 pt-0.5">
              <div className="flex items-center gap-1 text-[8px] font-black uppercase text-muted-foreground bg-muted/40 px-2 py-0.5 rounded-md">
                <Layers className="h-2.5 w-2.5 opacity-50" />
                {log.module}
              </div>
            ...
            {/* Bottom Row: Timestamp and Action Menu */}
            <div className="flex items-center justify-between pt-2 border-t border-border/40 text-[9px] text-muted-foreground font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg hover:bg-muted transition-all">
        ...
        {logs.length === 0 && (
          <div className="py-16 text-center opacity-40 italic text-xs text-muted-foreground">
            No interaction traces recorded.
          </div>
        )}
      </div>
